...
class Text2MotionDatasetV3(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        self.pointer = 0
        self.max_motion_length = opt.max_motion_length
        min_motion_len = 40 if self.opt.dataset_name =='trumans' else 24
        ...
        for name in tqdm(id_list):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                if len(motion) >= 200:
                    over_len=int((len(motion)-190)/2)
                    motion = motion[over_len:-over_len]
                if (len(motion)) < min_motion_len or (len(motion) >= 200):
                    print(len(motion))
                    continue
                text_data = []
...
class TextOnlyDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file):
        self.mean = mean
        self.std = std
        self.opt = opt
...
class Trumans(data.Dataset):
    def __init__(self, mode, datapath='./dataset/trumans_opt.txt', split="train", **kwargs):
        self.mode = mode
        ...
        self.split_file = pjoin(opt.data_root, f'{split}.txt')
        if mode == 'text_only':
            self.t2m_dataset = TextOnlyDatasetV2(self.opt, self.mean, self.std, self.split_file)
        else:
            self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'glove'), 'our_vab')
            self.t2m_dataset = Text2MotionDatasetV3(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer)
            self.num_actions = 1 # dummy placeholder
