...
smplx_male_model_45 = sk.SMPLXLayer(model_path='body_models/smplx', gender='male', num_betas=10, num_pca_comps=45)
smplx_neutral_model = sk.SMPLXLayer(model_path='body_models/smplx', gender='neutral', num_betas=10, num_pca_comps=-1)

def get_meshes_from_smplx(body_model_neutral=None, param=None, rh_pose=None):
...
def compute_optimization_loss(opt_joints, joints, opt_params=None):
    ...
    rw_loss = torch.mean((opt_joints[76, j-1, :] - joints[76, j-1, :]) ** 2) # 只限制关键帧腕关节的rw_loss
    loss += joints_loss
    ...
    if opt_params is not None:
        ...
        rw_vel = opt_joints[77:79, j-1, :] - joints[76:78, j-1, :]
        rw_accl = rw_vel[1:] - rw_vel[:-1]
        rw_a_loss = torch.mean(rw_accl**2)
