...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        if args.dataset in ['kit', 'humanml', 'trumans'] and args.eval_during_training:
                ...
                if self.step % self.log_interval == 0:
                    for k,v in logger.get_current().dumpkvs().items():
                        if k == 'loss':
                            ...
                            # 使用 SummaryWriter 记录 loss
                            self.writer.add_scalar('Loss/train', v, self.step + self.resume_step)
