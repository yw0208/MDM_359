...
def main():
    ...
    for rep_i in range(args.num_repetitions):
        ...
        if model.data_rep == 'hml_vec':
            ...
            sample, sample_hand = recover_from_ric(sample, n_joints, True)
        ...
        all_hand_motions.append(sample_hand.cpu().numpy())
    ...
    all_hand_motions = all_hand_motions[:total_num_samples]  # [bs, njoints, 6, seqlen]
    ...
    np.save(npy_path,
            {'motion': all_motions, 'hand_motion': all_hand_motions, 'text': all_text, 'lengths': all_lengths,
...
def load_dataset(args, max_frames, n_frames):
    data = get_dataset_loader(name=args.dataset,
                              ...
                              split='test',
                              hml_mode='text_only')
    if args.dataset in ['kit', 'humanml', 'trumans']:
